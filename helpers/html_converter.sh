...
build_index_file(){
 ...
 local HTML_FILE="$(basename ${FILE%.txt}.html)"
 COULORLESS_FILE_LINE=$(head -1 $FILE | tail -n 1 | cut -c27-)
 if [[ "$HTML_HEADLINE" == ' ' ]]; then
 ...
 echo "<header>
    ...
    <div>
      <ul>
      	 $MENU_LIST" | tee -a "$HTML_PATH""/index.txt" >/dev/null
...
#  sed -i 's/<head>/<head><br><meta http-equiv="Refresh" content="5"><br><link rel="stylesheet" href="..\/..\/html-files\/style.css" type="text\/css"\/>/g' $(dirname "$html_file")/index.html
 sed -i 's/<head>/<head><br><link rel="stylesheet" href="..\/style.css" type="text\/css"\/>/g' "$HTML_PATH""/index.html"
}
...
build_collection_file(){
 ...
 sed -i 's/<\/pre>//g' "$HTML_PATH""/collection.html"
#  sed -i 's/<head>/<head><br><meta http-equiv="Refresh" content="5"><br><link rel="stylesheet" href="..\/..\/html-files\/style.css" type="text\/css"\/>/g' $(dirname "$html_file")/index.html
CSSLINE="<head><br><link rel=\"stylesheet\" href=\"$HTML_PATH\/style.css\" type=\"text\/css\"\/>"
 #sed -i "s/<head>/$CSSLINE/g" "$HTML_PATH""/collection.html"
 sed -i 's/<head>/<head><br><link rel="stylesheet" href="..\/style.css" type="text\/css"\/>/g' "$HTML_PATH""/collection.html"
}
...
build_report_files(){
 ...
 do
  ...
  sed -i 's/&gt;/>/g' "$HTML_PATH""/$HTML_FILE"
  sed -i 's/<head>/<head><br><link rel="stylesheet" href="..\/style.css" type="text\/css"\/>/g' "$HTML_PATH""/$HTML_FILE"
}
